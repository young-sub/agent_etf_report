...
- Re-running native holdings collection without a **HoldingsSnapshotRefresh** skips already stored exact `ETF + provider_query_date` **HoldingsSnapshots** before calling a SourceProvider, fails with refresh-required evidence when fetched duplicate rows differ, and must not silently overwrite stored holdings.
...
- `data/agent_treport/live-source/source-provider-operational/<provider>/` is the default operating location for SourceProvider-collected readiness and report inputs. Each provider cache owns `catalog/`, `focus_etf_set.json`, `holdings-history/`, and `security-master/` artifacts. The older mixed `data/agent_treport/live-source/holdings-history/` store remains commit-eligible compatibility/archive data and a reconciliation source, not the forward readiness/report default. Repeated run evidence, report artifacts, runtime databases, daily smoke summaries, and similar reproducible per-run outputs use rolling retention, defaulting to the latest 10 runs.
...
- SourceProvider-collected holdings follow the fixed provider-cache handoff sequence: provider-scoped **HoldingsHistoryStore** plus provider-owned **SecurityResolutionExport** -> latest comparison export -> normalized **OperationalHoldingsExport** plus **CollectionSummary** fingerprint -> **OperationalRunReadiness** -> **SignalIntelligenceReport**. Provider-specific raw structures and provenance stay behind source acquisition or source state boundaries.
...
- The forward native operator flow is `collect provider cache -> export latest comparison -> check operational readiness -> run report`; mixed canonical history and direct fixture holdings exports remain compatibility/archive paths, not the forward report/readiness default.
